...
프로젝트는 Tailwind CSS를 사용하며, CSS 변수를 Tailwind extend로 연결합니다.
...
<html>
  └─ <body font-size: 16px, font-family: Pretendard Variable...>
       ...
       │    └─ [최근질문 패널] class="relative" (width: 0 ↔ var(--sidebar-width) 토글)
...
```
<header class="fixed top-0 left-0 right-0 h-topbar-height shrink-0 md:flex z-[9900]">
  실측: height=64px, position=fixed, z-index=9900
  bg: rgba(0,0,0,0) → 실제 배경은 body/흰색

<nav class="mx-auto flex h-full w-full items-center justify-between px-4">
  실측: height=64px, padding=0 16px

로고 이미지: <img alt="cube">
로고 텍스트: "AI 하나로" (별도 span)

우측 버튼들:
  문의: <button class="bg-white p-2 w-10 h-10 border border-gray-300 rounded-full hover:bg-gray-50">
        실측: bg=rgb(255,255,255), w=40px, h=40px, border-radius=9999px
  알림: <button class="relative outline-none">
        실측: bg=rgba(0,0,0,0)
```
...
개별 메뉴 아이템:
  ...
  레이블: text-xs(12px), color=rgb(79,79,79)
  구조: 아이콘 이미지 + 텍스트 레이블
...
전체 삭제 버튼:
  ...
  실측: bg=rgb(255,255,255), color=rgb(107,114,128)
        border=1px solid rgb(107,114,128), border-radius=9999px
        font-size=10.4px, height=22px, padding=0 8px

날짜 구분자: "오늘" — text-secondary(#6e6f72), font-size=16px
...
접기 버튼:
  ...
  실측: bg=rgb(255,255,255), border=1px solid #d1d5db
        border-radius=6px, 32×32px
```
...
드롭다운 버튼 (사업장 선택):
  ...
  실측: border=1px solid #eaeaea, border-radius=100px
        color=rgb(55,65,81), height=38px, padding=8px 16px, font-size=16px
...
input 태그:
  ...
  실측: bg=rgba(0,0,0,0), border=0, font-size=16px
        padding-left=16px, height=56px, color=rgb(15,15,15)
        placeholder color: #cdcdcd
```

### C-6. Tab Group (홈 상단 탭 — 일반질의/인기질의/FAQ)
...
활성 탭 (일반질의):
  ...
  실측: bg=rgb(255,255,255), color=rgb(31,41,55)
...
### C-7. Category Chips (자금현황, 수시입출 등)
...
활성 상태:
  ...
  실측: bg=rgb(79,99,210), color=white
        border=1px solid #4f63d2, border-radius=10px
        padding=8px 14px, font-size=16px

비활성 상태:
  class="...bg-[#f5f7fa] border-[#e8edf7] hover:bg-[#4f63d2] hover:text-white"
  실측: bg=rgb(245,247,250), color=rgb(15,15,15)
        border=1px solid #e8edf7
```

### C-8. Suggestion List Items (추천 질문)
...
실측: bg=rgba(0,0,0,0), color=rgb(15,15,15)
      border-radius=8px, font-size=14px, padding=8px 12px
...
1위: class="w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold border bg-[#fef3c7] text-[#92400e]"
     실측: bg=rgb(254,243,199), color=rgb(146,64,14)
           28×28px, border-radius=8px, font-size=14px, font-weight=700
...
비활성 탭 (FAQ):
  ...
  실측: bg=rgb(245,247,250)
```

### C-11. Chat Message 스타일

```
사용자 쿼리 텍스트:
  class="flex items-center gap-x-2 group"
  실측: color=rgb(79,99,210) → --aicfo-purple, font-size=16px

타임스탬프:
  <p class="text-sm font-normal text-nowrap text-tertiary">
  실측: color=rgb(160,160,160), font-size=14px
...
### C-12. Report Card (보고서 카드)

```
카드 외부:
  class="overflow-hidden rounded-sm border border-gray-200 bg-white shadow-2xl"
  실측: bg=rgb(255,255,255), border=1px solid #e5e7eb, border-radius=2px
...
데이터 행 홀수 (tr):
  ...
  실측: bg=rgb(255,255,255), border-bottom=1px solid #f1f5f9
...
셀 (td):
  ...
  실측: color=rgb(30,41,59), font-size=14px
        padding=12px 16px, border-right=1px solid #f1f5f9
```
...
중요 인사이트 (emerald):
  ...
  실측: border-left=3px solid rgb(52,211,153)
...
Excel 다운로드:
  ...
  실측: bg=rgb(5,150,105), border-radius=6px
...
PDF 내보내기:
  ...
  실측: bg=white, color=rgb(55,65,81)
        border=1px solid #e5e7eb, border-radius=8px
        font-size=12px, font-weight=500, height=34px
```
...
테이블 헤더 (TH):
  ...
  실측: bg=rgb(78,103,131), color=white
        font-size=11.9px, font-weight=700, padding=0 10px
...
### C-19. Buttons 전체 변형 실측 요약

| # | 용도 | bg | color | border | border-radius | font-size | 크기 |
|---|---|---|---|---|---|---|---|
...
| 6 | Ghost Circle (헤더 아이콘) | white | #0f0f0f | #d1d5db | 9999px | 16px | 40×40 |
| 7 | 페이지네이션 활성 | #3b82f6 | white | #3b82f6 | 4px | 14px | 32×32 |
| 8 | 페이지네이션 비활성 | transparent | #d1d5db | #e5e7eb | 4px | 14px | 32×32 |
| 9 | 페이지네이션 이전/다음 | white | — | #d1d5db | 6px | — | 32×32 |
